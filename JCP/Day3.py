my_input = """.........232.633.......................803..........................361................192............539.................973.221...340.....
...
def copy_digit(grid, x, y, result):
  dx = -1
  while get_cell(grid, x+dx, y) and get_cell(grid, x+dx, y).isdigit():
    previous = get_cell(grid, x+dx, y)
    result[y][x+dx] = previous
    dx -= 1

  dx = 1
  while get_cell(grid, x+dx, y) and get_cell(grid, x+dx, y).isdigit():
    next = get_cell(grid, x+dx, y)
    result[y][x+dx] = next
    dx += 1

def copy_neighbors(grid, x, y, result):
  for dx in [-1, 0, 1]:
    for dy in [-1, 0, 1]:
      cell = get_cell(grid, x+dx, y+dy)
      if cell:
        result[y+dy][x+dx] = cell

        if cell.isdigit():
          copy_digit(grid, x+dx, y+dy, result)
  return result

def get_digit(grid, x, y):
  dx = -1
  while get_cell(grid, x+dx, y) and get_cell(grid, x+dx, y).isdigit():
    dx -= 1
  dx += 1
  result = 0
  while get_cell(grid, x+dx, y) and get_cell(grid, x+dx, y).isdigit():
    result = result *10 + int(get_cell(grid, x+dx, y))
    grid[y][x+dx] = '.'
    dx += 1
  return result

def find_neighbor_numbers(grid, x, y):
  result = []
  for dx in [-1, 0, 1]:
    for dy in [-1, 0, 1]:
      cell = get_cell(grid, x+dx, y+dy)
      if cell and cell.isdigit():
          result.append(get_digit(grid, x+dx, y+dy))
  return result


def prepare_result(input, symbol):
  ...
  for y in range(len(grid)):
    for x in range(len(grid[0])):
      if is_symbol(grid, x, y):
        if not symbol or symbol == get_cell(grid, x, y):
          copy_neighbors(grid, x, y, result)
  return grid, result

def part1(input, symbol=None):
  grid, result = prepare_result(input, symbol)
  
  for y in range(len(result)):
    for x in range(len(result[0])):
      if not get_cell(result, x, y).isdigit():
        result[y][x] = " "
  
  total = 0
  for y in range(len(result)):
    total += sum([int(x) for x in "".join(result[y]).split(" ") if x])
  return total

def part2(input):
  grid, result = prepare_result(input, "*")

  total = 0
  for y in range(len(result)):
    for x in range(len(result[0])):
      if is_symbol(result, x, y):
        numbers = find_neighbor_numbers(result, x, y)
        if len(numbers) == 2:
...
print("Part 1 from example : " + str(part1(example)))
print("Part 1 from my puzzle : " + str(part1(my_input)))

print("Part 2 from example : " + str(part2(example)))
print("Part 2 from input : " + str(part2(my_input)))
