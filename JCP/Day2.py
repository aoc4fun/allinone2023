...
input = """Game 1: 2 red, 2 green; 1 red, 1 green, 2 blue; 3 blue, 3 red, 3 green; 1 blue, 3 green, 7 red; 5 red, 3 green, 1 blue
...
def parse_game(game):
  ...
  pulls = pulls.split(";")
  pulls = [pull.split(",") for pull in pulls]
  return game_id, pulls

def parse_color(color_pull):
  color_pull = color_pull.strip().split(" ")
  number = int(color_pull[0])
  color = color_pull[1]
  return number, color
...
def part1(input):
  ...
  for game in input.split("\n"):
    game_id, pulls = parse_game(game)
    too_much = False
    for pull in pulls:
      for color_pull in pull:
        number, color = parse_color(color_pull)
        ...
        if color == 'blue' and number > 14:
          too_much = True
    if not too_much:
      print(f"Game {game_id}: {pulls}")    
      result += int(game_id)
  return result

result = part1(example)
print("Result for example1 is : ", result)

result = part1(input)
print("Result for part1 is : ", result)
...
def part2(input):
  ...
  for game in input.split("\n"):
    ...
    for pull in pulls:
      for color_pull in pull:
        number, color = parse_color(color_pull)
        minimums.setdefault(color, 0);
        minimums[color] = max(minimums[color], number)
    
    print(f"Game {game_id}: {minimums}")    
    result += np.prod(list(minimums.values()))
  return result

result = part2(example)
print("Result for example2 is : ", result)

result = part2(input)
print("Result for input2 is : ", result)
