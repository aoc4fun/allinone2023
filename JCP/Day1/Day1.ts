...
function findFirstNumber(line){
    if(!isNaN(parseInt(line[0]))) return line[0];
    ...
    return findFirstNumber(line.slice(1));
}

function findLastNumber(line){
    if(!isNaN(parseInt(line[line.length - 1]))) return line[line.length - 1];
    ...
    if(line.endsWith("seven")) return '7';
    if(line.endsWith("eight")) return '8';
    if(line.endsWith("nine")) return '9';
    return findLastNumber(line.slice(0, -1));
}
