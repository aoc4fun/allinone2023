...
my_input = """F--FJ77F-L7---J-F777-F7|77F|-J-F|77|F-L-J-77-L7FL77-7|----.F7-|-|.7-.-FL-FF-FF77.|.F-F-.F|.7.F.7-F-77.J7|----7LL7F|7|-F-J-777-77.F-7-FL77FF|
...
def findPipeDestination(input, x, y):
  north = (x, y-1)
  south = (x, y+1)
  east = (x+1, y)
  west = (x-1, y)
  ...
  letter = input.splitlines()[y][x]
  if letter == '.':
    return []
  if letter == 'S':
    res = []
    for n in [north, south, east, west]:
      if (x,y) in findPipeDestination(input, n[0], n[1]):
        res.append(n)
    return res
  return pipes[letter]

def keep_only_visited(input, visited):
  res = ""
  lines = input.splitlines()
  for y in range(len(lines)):
    line = input.splitlines()[y]
...
def expand_flood(arr, positions):
...
def flood(input):
  arr = [list(x) for x in input.splitlines()]
  result = expand_flood(arr, [(0, 0)])
  return "\n".join(["".join(x) for x in arr])
  
def visit_path(input):
  start = next = findStart(input)
  visited = [start]
  
  while True:
    #print(next)
    nexts = findPipeDestination(input, next[0], next[1])
    if nexts[0] not in visited:
      next = nexts[0]
    else:
      if nexts[1] not in visited:
        next = nexts[1]
      else:
        if start in nexts:
          break
...
def replace_start(input):
  ...
  start_neighbors = findPipeDestination(input, x, y)
  for other in pipes.keys():
    other_neighbors = pipes[other]
    if(start_neighbors[0] in other_neighbors and start_neighbors[1] in other_neighbors):
      return input.replace('S', other)

def count_enclosed(input):
  count = 0
  for (y, line) in enumerate(input.splitlines()):
    for(x, char) in enumerate(line):
      if char == '.' and x % 2 == 0 and y % 2 == 0:
        count += 1 
  return count

def part2(input):
  visited = visit_path(input)
  new_input = keep_only_visited(input, visited)
  new_input = replace_start(new_input)
  new_input = double_columns(new_input)
  new_input = double_rows(new_input)
  flooded = flood(new_input)
  return count_enclosed(flooded)
  
  
print("Furthest point in part1 example : " + str(part1(example)))
print("Furthest point in part1 my input : " + str(part1(my_input)))

print("Enclosed spaces part2 example : " + str(part2(example2)))
print("Enclosed spaces part2 my input : " + str(part2(my_input)))
